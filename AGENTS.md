...
> **Single-file-reference per coverage-locked `.vue` (coverage-merge invariant)**: A `.vue` covered by a 100% glob threshold may be executed by **only ONE** test file — and "execute" includes **bare imports**, **lazy-route chunk loaders**, and **whole-app mounts**, not just deep `mount()`. Each test file runs in its own worker isolate and any file that imports the component produces a local v8 function map whose ids start from 1; maps with **different function counts** (a file that never renders a template branch yields a shorter map — e.g. 17 vs 19 functions) shift ids, and at full 299-file scale istanbul merges those maps **by id**, landing counts on the wrong handlers → `functions` drops below the 100% threshold. History (LoginEnhanced.vue, functions 88.23% on Linux CI; Windows/Node25 stayed green — OS-dependent v8 fn-id order, so Windows green is NOT a pass): each of these was a real second executor until removed/stubbed, in order — ① redundant deep-mount in `AuthViewsBatch.test.ts`, ② **bare import** in `smoke.test.ts`, ③ lazy-route chunk loader + `push('/login')` in `router/router-index.test.ts` (stub `LoginEnhanced` there), ④ `promptContract.test.ts` doing `await import('@/main')` — the real app mount resolves `/login` and executes the component (stub `LoginEnhanced` there too). Root-cause proof: a module-load marker inside the component fires exactly **N** times for N referencing test files; after all four were fixed, WSL/Node22 full `--coverage` shows functions/branches 100% with a single load. So: grep ALL test files for any reference (import, router push, `import('@/main')`, chunk loaders) and keep only the dedicated test. `maxWorkers:1` does NOT fix this (merge happens at the per-file-shard level). Full mechanism: `frontend/vitest.config.ts` comment **(A)**.
>
> **Second face of the same symptom (2026-09-05)**: after the merge-conflict face was fixed, the same file still gated at **99.91% functions / 99.99% lines** — this remainder was a REAL coverage gap, not a merge artifact: v-model on a native `<input>` compiles an `onUpdate:modelValue._cache` handler that fires **only on real DOM input events**. Tests that set `vm.totpCode = '...'` directly (script-level assignment) never trigger it → the handler stays count-0 in every shard, honestly. Fix: add a real-DOM case (`find('input...').setValue('654321')` in the dedicated test). Rule: when a coverage-locked `.vue` shows 99.9x%, inspect `coverage/coverage-final.json` for the exact missed fn/statement ids BEFORE assuming a merge phantom — `vm.x =` assignments bypass v-model handlers and leave genuine holes.
