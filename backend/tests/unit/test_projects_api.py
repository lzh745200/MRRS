...
def sample_project():
    ...
    p.village_id = 1
    p.organization_id = 1
    p.budget = Decimal("100.00")
    p.actual_cost = Decimal("50.00")
    p.invested_amount = Decimal("30.00")
    p.progress = 50
    p.leader = "领导"
    p.contact = "123"
    p.responsible_unit = "单位"
    p.responsible_person = "张三"
    p.contact_phone = "13800138000"
    p.start_date = date(2026, 1, 1)
    p.end_date = date(2026, 12, 31)
    p.actual_start_date = date(2026, 2, 1)
    p.actual_end_date = None
    p.urgency_level = "normal"
    p.contract_number = "CT-001"
    p.fund_manager = "李四"
    p.fund_usage_plan = "计划"
    p.fund_source = "superior_allocation"
    p.payer_account_name = "payer"
    p.payer_account_number = "123"
    p.payer_bank = "银行"
    p.payer_handler = "经办人"
    p.payer_contact = "139"
    p.payee_account_name = "payee"
    p.payee_bank = "银行2"
    p.payee_handler = "经办人2"
    p.payee_contact = "140"
    p.is_delayed = False
    p.delay_reason = None
    p.expected_benefits = "效益"
    p.achievements = "成果"
    p.tags = "tag1,tag2"
    p.remarks = "备注"
    p.created_by = 1
...
def sample_fund():
    ...
    f.name = "经费1"
    f.amount = 100.0
    f.approved_amount = 90.0
    f.used_amount = 50.0
    f.deviation_rate = 5.0
    f.health_score = 85
    f.source = "财政"
    f.purpose = "用途"
...
class TestUtilityFunctions:
    ...
    def test_project_create_valid(self):
        d = ProjectCreate(
            name="T", budget=100.0, start_date="2026-01-01", end_date="2026-12-31"
        )
        assert d.name == "T"
...
class TestProjectsAPI:
    ...
    def test_list_projects(self, client, mock_db, admin_user):
        ...
        p.village_id = None
        p.organization_id = 1
        p.budget = Decimal("100")
        p.actual_cost = Decimal("0")
        p.progress = 0
        p.invested_amount = Decimal("0")
        p.responsible_unit = "U"
        p.responsible_person = "P"
        ...
        with patch("sqlalchemy.orm.selectinload"), patch(
            "app.core.data_permission.filter_by_data_scope", return_value=q
        ):
    ...
    def test_create_project(self, mock_ip, client, mock_db, admin_user):
        _setup_client(client, mock_db, admin_user)
        with patch("app.api.v1.projects.AuditLogService") as mock_al, patch(
            "app.api.v1.projects.AuditEnhancementService"
        ) as mock_ae, patch("app.api.v1.projects.write_work_log"):
    ...
    def test_create_project_with_progress(self, mock_ip, client, mock_db, admin_user):
        ...
        added_projects = [
            c[0][0] for c in mock_db.add.call_args_list if isinstance(c[0][0], Project)
        ]
    ...
    @patch("app.api.v1.projects.get_client_ip", return_value="127.0.0.1")
    def test_create_project_default_progress_zero(
        self, mock_ip, client, mock_db, admin_user
    ):
        """不传 progress 时默认为 0"""
        _setup_client(client, mock_db, admin_user)
        with patch("app.api.v1.projects.AuditLogService") as mock_al, patch(
    ...
    def test_create_project_progress_out_of_range(self, client, mock_db, admin_user):
        ...
        for bad in (-1, 101):
            resp = client.post(
                "/api/v1/projects", json={"name": "越界进度", "progress": bad}
            )
    ...
    def test_create_project_duplicate_code(self, client, mock_db, admin_user):
        _setup_client(client, mock_db, admin_user)
        existing = MagicMock()
        existing.code = "PRJ-EXISTING"
        mock_db.query.return_value.filter.return_value.first.return_value = existing
        resp = client.post(
            "/api/v1/projects", json={"name": "重复", "code": "PRJ-EXISTING"}
        )
    ...
    def test_update_project_forbidden(
        self, client, mock_db, regular_user, sample_project
    ):
    ...
    def test_update_project_date_validation(
        self, client, mock_db, admin_user, sample_project
    ):
        _setup_client(client, mock_db, admin_user)
        ...
        resp = client.put(
            "/api/v1/projects/1",
            json={"start_date": "2026-12-01", "end_date": "2026-01-01"},
        )
        assert resp.status_code == 500  # AppError → handle_db_errors_async → 500

    @patch("app.api.v1.projects.get_client_ip", return_value="127.0.0.1")
    def test_update_project_status_change(
        self, mock_ip, client, mock_db, admin_user, sample_project
    ):
        _setup_client(client, mock_db, admin_user)
        with patch("app.api.v1.projects.AuditLogService") as mock_al, patch(
            "app.api.v1.projects.AuditEnhancementService"
        ) as mock_ae, patch("app.api.v1.projects.write_work_log"), patch(
            "app.services.fund_event_handler.on_project_status_change"
        ) as mock_psc:
            mock_al.return_value.log = AsyncMock()
            mock_ae.create_audit_log.return_value = MagicMock()
            mock_db.query.return_value.filter.return_value.options.return_value.first.return_value = (
    ...
    @patch("app.api.v1.projects.get_client_ip", return_value="127.0.0.1")
    def test_update_project_status_change_rollback(
        self, mock_ip, client, mock_db, admin_user, sample_project
    ):
        _setup_client(client, mock_db, admin_user)
        with patch(
    ...
    def test_delete_project_already_cancelled(
        self, client, mock_db, admin_user, sample_project
    ):
        _setup_client(client, mock_db, admin_user)
    ...
    def test_get_project_change_history(
        self, client, mock_db, admin_user, sample_project
    ):
        _setup_client(client, mock_db, admin_user)
        with patch(
            "app.api.v1.projects.AuditEnhancementService.get_change_history",
    ...
    def test_get_project_funds(
        self, client, mock_db, admin_user, sample_project, sample_fund
    ):
        _setup_client(client, mock_db, admin_user)
        fund_q = MagicMock()
        fund_q.count.return_value = 1
        fund_q.filter.return_value.count.return_value = 1
        fund_q.filter.return_value.order_by.return_value.offset.return_value.limit.return_value.all.return_value = [
            sample_fund
        ]
        proj_q = MagicMock()
    ...
    def test_create_project_fund(self, client, mock_db, admin_user, sample_project):
        ...
        resp = client.post(
            "/api/v1/projects/1/funds", json={"name": "新增经费", "amount": 50.0}
        )
        assert resp.status_code == 201

    def test_create_project_fund_fail(
        self, client, mock_db, admin_user, sample_project
    ):
        _setup_client(client, mock_db, admin_user)
        mock_db.query.return_value.filter.return_value.options.return_value.first.return_value = (
        ...
        resp = client.post(
            "/api/v1/projects/1/funds", json={"name": "经费", "amount": 50.0}
        )
        assert resp.status_code == 500

    def test_get_project_tasks(
        self, client, mock_db, admin_user, sample_project, sample_task
    ):
        _setup_client(client, mock_db, admin_user)
        task_q = MagicMock()
        task_q.filter.return_value.first.return_value = sample_project
        task_q.filter.return_value.count.return_value = 1
        task_q.filter.return_value.order_by.return_value.offset.return_value.limit.return_value.all.return_value = [
            sample_task
        ]
        proj_q = MagicMock()
        proj_q.filter.return_value.first.return_value = sample_project
        mock_db.query.side_effect = lambda model: (
            task_q if model == ProjectTask else proj_q
        )
    ...
    def test_get_project_tasks_filtered(
        ...
        q2.order_by.return_value.offset.return_value.limit.return_value.all.return_value = [
            sample_task
        ]
    ...
    def test_create_project_task_fail(
        self, client, mock_db, admin_user, sample_project
    ):
        _setup_client(client, mock_db, admin_user)
        mock_db.query.return_value.filter.return_value.options.return_value.first.return_value = (
    ...
    def test_update_project_task(
        ...
        _setup_client(client, mock_db, admin_user)
        q1 = MagicMock()
        q1.filter.return_value.first.return_value = sample_project
        q2 = MagicMock()
        q2.filter.return_value.first.return_value = sample_task
        mock_db.query.side_effect = lambda m: q1 if m == Project else q2
        resp = client.put(
            "/api/v1/projects/1/tasks/1",
            json={"name": "更新任务", "status": "completed"},
        )
        assert resp.status_code == 200

    def test_update_project_task_not_found(
        self, client, mock_db, admin_user, sample_project
    ):
        _setup_client(client, mock_db, admin_user)
        q1 = MagicMock()
    ...
    def test_update_project_task_fail(
        self, client, mock_db, admin_user, sample_project, sample_task
    ):
        _setup_client(client, mock_db, admin_user)
        q1 = MagicMock()
    ...
    def test_delete_project_task_not_found(
        self, client, mock_db, admin_user, sample_project
    ):
        _setup_client(client, mock_db, admin_user)
        q1 = MagicMock()
    ...
    def test_delete_project_task_fail(
        self, client, mock_db, admin_user, sample_project, sample_task
    ):
        _setup_client(client, mock_db, admin_user)
        q1 = MagicMock()
    ...
    def test_export_projects_with_filters(self, client, mock_db, admin_user):
        ...
        with patch("sqlalchemy.orm.selectinload"), patch(
            ...
            mock_db.query.return_value = q
            resp = client.get(
                "/api/v1/projects/export?keyword=test&project_type=infra&status=draft"
            )
        assert resp.status_code == 200
    ...
    def test_export_projects_csv_fallback(self, client, mock_db, admin_user):
        ...
        p.status = "S"
        p.budget = None
        p.invested_amount = None
        p.progress = None
        p.responsible_person = None
        p.responsible_unit = None
        p.start_date = None
        p.end_date = None
        q = MagicMock()
        q.filter.return_value = q
    ...
    def test_import_projects_rate_limited(self, client, mock_db, admin_user):
        ...
        with patch("app.api.v1.projects.check_rate_limit", mock_check):
            resp = client.post(
                "/api/v1/projects/import", files={"file": ("test.xlsx", b"data", "")}
            )
    ...
    def test_import_projects_parse_error(self, client, mock_db, admin_user):
        ...
        with patch("app.api.v1.projects.check_rate_limit", mock_check):
            resp = client.post(
                "/api/v1/projects/import",
                files={"file": ("test.xlsx", b"not-excel", "")},
            )
        assert resp.status_code == 400
    ...
    def test_upload_project_files(self, client, mock_db, admin_user, sample_project):
        ...
        try:
            ...
            with patch("app.core.config.settings", mock_settings), patch(
                "app.utils.upload_helper.settings", mock_settings
            ):
                resp = client.post(
    ...
    def test_upload_project_files_invalid_category(
        self, client, mock_db, admin_user, sample_project
    ):
        _setup_client(client, mock_db, admin_user)
        mock_db.query.return_value.filter.return_value.options.return_value.first.return_value = (
    ...
    def test_upload_project_files_type_check(
        self, client, mock_db, admin_user, sample_project
    ):
        _setup_client(client, mock_db, admin_user)
        mock_db.query.return_value.filter.return_value.options.return_value.first.return_value = (
    ...
    def test_upload_project_files_forbidden(
        self, client, mock_db, regular_user, sample_project
    ):
        _setup_client(client, mock_db, regular_user)
        sample_project.created_by = 1
    ...
    def test_list_project_files(
        self, client, mock_db, admin_user, sample_project, sample_project_file
    ):
        _setup_client(client, mock_db, admin_user)
        q1 = MagicMock()
        q1.filter.return_value.first.return_value = sample_project
        q2 = MagicMock()
        q2.filter.return_value = q2
        q2.order_by.return_value.all.return_value = [sample_project_file]
        mock_db.query.side_effect = lambda m: q1 if m == Project else q2
        resp = client.get("/api/v1/projects/1/files")
        assert resp.status_code == 200 and len(resp.json()["data"]["files"]) == 1

    def test_list_project_files_filtered(
        self, client, mock_db, admin_user, sample_project, sample_project_file
    ):
        _setup_client(client, mock_db, admin_user)
        q1 = MagicMock()
    ...
    def test_delete_project_file(
        ...
        q1.filter.return_value.first.return_value = sample_project
        q2 = MagicMock()
        q2.filter.return_value.first.return_value = sample_project_file
        mock_db.query.side_effect = lambda m: q1 if m == Project else q2
        with patch("app.api.v1.projects.os.path.exists", return_value=True), patch(
            "app.api.v1.projects.os.remove"
        ):
            resp = client.delete("/api/v1/projects/1/files/1")
        assert resp.status_code == 200

    def test_delete_project_file_oserror(
        self, client, mock_db, admin_user, sample_project, sample_project_file
    ):
        _setup_client(client, mock_db, admin_user)
        q1 = MagicMock()
        ...
        mock_db.query.side_effect = lambda m: q1 if m == Project else q2
        with patch("app.api.v1.projects.os.path.exists", return_value=True), patch(
            "app.api.v1.projects.os.remove", side_effect=OSError
        ):
            resp = client.delete("/api/v1/projects/1/files/1")
        assert resp.status_code == 200

    def test_delete_project_file_not_found(
        self, client, mock_db, admin_user, sample_project
    ):
        _setup_client(client, mock_db, admin_user)
        q1 = MagicMock()
    ...
    def test_delete_project_file_forbidden(
        self, client, mock_db, regular_user, sample_project
    ):
        _setup_client(client, mock_db, regular_user)
        sample_project.created_by = 1
    ...
    def test_download_project_file(self, client, mock_db, admin_user):
        ...
        try:
            ...
            pf.id = 1
            pf.project_id = 1
            pf.category = "research"
            pf.filename = "test.pdf"
            pf.filepath = tf_path
            pf.file_size = 7
            pf.uploaded_by = 1
    ...
    def test_download_project_file_missing_on_disk(
        self, client, mock_db, admin_user, sample_project_file, sample_project
    ):
        _setup_client(client, mock_db, admin_user)
        # _get_project_or_404 走 options 链返回 Project；ProjectFile 查询（无 options）返回文件记录
        q_project = MagicMock()
        q_project.filter.return_value.options.return_value.first.return_value = (
            sample_project
        )
        q_file = MagicMock()
