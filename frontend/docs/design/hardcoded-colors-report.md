...
> 生成：`check_hardcoded_styles.py --update-baseline` ｜ 存量 **768 处 / 105 文件**（基线已冻结，只减不增）
> 消化方式：按映射表替换为 `var(--token)` → 跑 vitest+vue-tsc → `--update-baseline` 收紧基线。

## 颜色频次 Top15（映射表依据）

> Top15 字面量合计 379/768 处（约 49%），映射直换即可消化大半。
...
| 15 | `frontend/src/views/auth/GetMachineCode.vue` |
| 15 | `frontend/src/views/funds/Report.vue` |
| 14 | `frontend/src/views/funds/Budget.vue` |
| 12 | `frontend/src/views/auth/ForgotPassword.vue` |
