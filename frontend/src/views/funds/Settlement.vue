<template>
  <div class="settlement-container">
    ...
    <!-- 无项目参数（菜单直接进入）：项目选择视图 -->
    <el-card v-if="invalidProject" class="mt-4" shadow="never">
      <el-empty description="请选择项目以查看决算与绩效">
...
function goSelectedProject() {
  ...
  pushSafe(`/funds/settlement/${selectedProjectId.value}`)
}
...
async function loadData() {
  if (invalidProject.value) {
    // 无项目参数（菜单直接进入）→ 展示项目选择视图，不再弹回经费总览
