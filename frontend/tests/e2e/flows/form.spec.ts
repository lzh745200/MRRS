...
test.describe('表单提交流程', () => {
  ...
  test.describe('村庄数据管理', () => {
    test('可以访问村庄列表页', async ({ page }) => {
      // 导航到村庄管理页面（/villages 重定向到 /supported-villages）
      await page.goto('/villages')

      // 验证页面加载：列表页无 h1/h2/.page-title 标题元素，改断言真实存在的稳定元素
      await expect(page).toHaveURL(/\/supported-villages/, { timeout: 10000 })
      await expect(page.getByRole('button', { name: '新增帮扶村' })).toBeVisible({ timeout: 10000 })
    })
    ...
    test('表单验证正常工作', async ({ page }) => {
      ...
      if (await addButton.isVisible()) {
        ...
        const submitButton = page.locator(
          'button:has-text("确定"), button:has-text("保存"), button:has-text("提交")'
        )
  ...
  test.describe('项目数据管理', () => {
    test('可以访问项目列表页', async ({ page }) => {
      ...
      // 验证页面加载
      await expect(page.locator('h1, h2, .page-title').filter({ hasText: /项目/ })).toBeVisible({
        timeout: 5000,
      })
    })
    ...
    test('可以查看项目详情', async ({ page }) => {
      ...
      const viewButton = page
        .locator('button:has-text("查看"), button:has-text("详情"), .el-table__row')
        .first()
      if (await viewButton.isVisible()) {
        await viewButton.click()

        // 验证详情页面或弹窗出现
        await expect(page.locator('.el-dialog, .detail-page, .project-detail')).toBeVisible({
          timeout: 3000,
        })
      }
    })
  ...
  test.describe('学校数据管理', () => {
    test('可以访问学校列表页', async ({ page }) => {
      ...
      // 验证页面加载
      await expect(page.locator('h1, h2, .page-title').filter({ hasText: /学校/ })).toBeVisible({
        timeout: 5000,
      })
    })
  })
