  ...
  test.describe('提交审批流程', () => {
    test('可以从数据详情页提交审批', async ({ page }) => {
      ...
      if (await editButton.isVisible()) {
        ...
        // 查找提交审批按钮
        const submitApprovalButton = page.locator(
          'button:has-text("提交审批"), button:has-text("申请审批")'
        )
  ...
  test.describe('审批通过流程', () => {
    test('可以打开审批通过对话框', async ({ page }) => {
      ...
      if (await approveButton.isVisible()) {
        ...
        // 验证审批对话框出现
        const dialog = page.locator('.el-overlay:visible')
  ...
  test.describe('审批拒绝流程', () => {
    ...
    test('审批拒绝必须填写原因', async ({ page }) => {
      ...
      if (await rejectButton.isVisible()) {
        ...
        // 不填写原因直接点击确认
        const confirmButton = page
          .locator('button:has-text("确认拒绝"), button:has-text("确定")')
          .last()
        if (await confirmButton.isVisible()) {
          await confirmButton.click()

          // 等待可能的验证错误提示出现（存在与否取决于表单校验实现）
          await page.waitForTimeout(500)
        }
      }
    })
  ...
  test.describe('审批转交流程', () => {
    test('可以打开转交对话框', async ({ page }) => {
      ...
      const transferButton = page
        .locator('button:has-text("转交"), button:has-text("转办")')
        .first()
    ...
    test('转交需要选择目标用户', async ({ page }) => {
      ...
      if (await transferButton.isVisible()) {
        ...
        // 等待转交对话框出现（可见 overlay 内的选择器，避免命中页面筛选栏）
        const dialog = page.locator('.el-overlay:visible')
        await expect(dialog.first()).toBeVisible({ timeout: 10000 })

        // 验证用户选择器存在（限定对话框内，页面筛选栏有多个同名 combobox）
        const userSelector = dialog.locator('.el-select, [role="combobox"]').first()
    ...
    test('可以执行转交操作', async ({ page }) => {
      ...
      if (await transferButton.isVisible()) {
        ...
        // 等待转交对话框出现
        const dialog = page.locator('.el-overlay:visible').first()
        await expect(dialog).toBeVisible({ timeout: 10000 })

        // 选择目标用户（限定对话框内，避免命中页面筛选栏）
        const userSelector = dialog.locator('.el-select, [role="combobox"]').first()
        if (await userSelector.isVisible()) {
          await userSelector.click()

          // 选择第一个选项
          const option = page.locator('.el-select-dropdown__item:visible, [role="option"]:visible').first()
        ...
        const confirmButton = dialog
          .locator('button:has-text("确认转交"), button:has-text("确定")')
          .last()
        if (await confirmButton.isVisible()) {
          // 监听API响应
          const responsePromise = page
  ...
  test.describe('变更对比视图', () => {
    test('可以查看变更对比', async ({ page }) => {
      ...
      const diffButton = page
        .locator('button:has-text("对比"), button:has-text("查看变更")')
        .first()
    ...
    test('变更对比显示原值和新值', async ({ page }) => {
      ...
      if (await diffButton.isVisible()) {
        ...
        // 等待对比对话框出现（变更对比需先拉取数据，等待可见 overlay）
        const dialog = page.locator('.el-overlay:visible').first()
        await expect(dialog).toBeVisible({ timeout: 10000 })

        // 验证对比表格存在（限定对话框内，页面主表格会造成 strict 冲突）
        const diffTable = dialog.locator('.el-table, table').first()
        if (await diffTable.isVisible()) {
          // 原值/新值列随数据动态渲染，此处不强制断言
        }
      }
    })
  })
