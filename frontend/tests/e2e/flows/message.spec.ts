...
test.describe('消息通知系统', () => {
  ...
  test.describe('消息中心', () => {
    test('可以访问消息中心页面', async ({ page }) => {
      // 导航到消息中心（真实路由为 /message；旧的 /messages 已不存在，会落到 404）
    ...
    test('消息列表正确显示', async ({ page }) => {
      ...
      // 验证表格有列头
      const headers = messageTable.locator('th')
  ...
  test.describe('消息标记已读', () => {
    ...
    test('可以单独标记消息为已读', async ({ page }) => {
      ...
      const markReadButton = page
        .locator('button:has(.el-icon-check), button[aria-label*="已读"]')
        .first()
    ...
    test('可以全部标记为已读', async ({ page }) => {
      ...
      const markAllReadButton = page.locator(
        'button:has-text("全部已读"), button:has-text("全部标记")'
      )
  ...
  test.describe('消息删除', () => {
    test('可以删除单条消息', async ({ page }) => {
      ...
      const deleteButton = page
        .locator('button:has(.el-icon-delete), button[aria-label*="删除"]')
        .first()
    ...
    test('可以批量删除消息', async ({ page }) => {
      ...
      if (count > 1) {
        ...
        // 查找批量删除按钮
        const batchDeleteButton = page.locator(
          'button:has-text("删除选中"), button:has-text("批量删除")'
        )
  ...
  test.describe('通知偏好设置', () => {
    // 应用没有独立的“通知设置”页面：/notifications/settings 路由不存在，
    // 通知偏好仅在 API 层定义（src/api/message.ts）而无对应 UI。
    // 消息中心顶部的筛选表单（按消息类型/已读状态控制通知展示）是真实存在的
    // 等价功能，因此以下用例改写为断言该筛选区。
    test('消息中心显示通知筛选功能区', async ({ page }) => {
      await page.goto('/message')

      // 验证筛选表单及其两个下拉（消息类型 / 状态）存在
      const filterForm = page.locator('.message-center .el-form').first()
      await expect(filterForm).toBeVisible({ timeout: 10000 })
      await expect(filterForm.locator('.el-select')).toHaveCount(2)
    })

    test('消息类型筛选包含各类通知选项', async ({ page }) => {
      await page.goto('/message')

      // 打开“消息类型”筛选下拉
      const typeFilter = page
        .locator('.message-center .el-form-item')
        .filter({ hasText: '消息类型' })
        .locator('.el-select')
      await expect(typeFilter).toBeVisible({ timeout: 10000 })
      await typeFilter.click()

      // 验证各类通知选项（系统/审批/任务/备份）。下拉被 teleport 到 body，
      // 每个 select 各渲染一份选项，用 :visible 限定当前打开的下拉避免多处命中
      ...
      await expect(options.filter({ hasText: '系统通知' })).toBeVisible()
      await expect(options.filter({ hasText: '审批通知' })).toBeVisible()
      await expect(options.filter({ hasText: '任务提醒' })).toBeVisible()
    })
  ...
  test.describe('消息类型', () => {
    test('系统通知正确显示', async ({ page }) => {
      ...
      if (await typeFilter.isVisible()) {
        ...
        const systemOption = page.locator(
          '.el-select-dropdown__item:has-text("系统"), option:has-text("系统")'
        )
    ...
    test('审批通知正确显示', async ({ page }) => {
      ...
      if (await typeFilter.isVisible()) {
        ...
        // 选择审批通知
        const approvalOption = page.locator(
          '.el-select-dropdown__item:has-text("审批"), option:has-text("审批")'
        )
    ...
    test('任务提醒正确显示', async ({ page }) => {
      ...
      if (await typeFilter.isVisible()) {
        ...
        const taskOption = page.locator(
          '.el-select-dropdown__item:has-text("任务"), option:has-text("任务")'
        )
