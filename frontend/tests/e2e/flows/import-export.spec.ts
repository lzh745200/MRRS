...
test.describe('数据导入导出流程', () => {
  ...
  test.describe('模板下载流程', () => {
    test('可以访问数据导入页面', async ({ page }) => {
      // 导航到数据导入页面（真实路由为 /import/data）
      await page.goto('/import/data')

      // 验证页面加载 - 检查页面标题（DataImport.vue 的 h2）
      await expect(page.getByRole('heading', { name: '数据导入' })).toBeVisible({ timeout: 10000 })
    })
    ...
    test('可以下载导入模板', async ({ page }) => {
      ...
      const downloadButton = page.locator(
        'button:has-text("下载"), button:has-text("模板"), a:has-text("模板")'
      )
  ...
  test.describe('数据导入流程', () => {
    test('可以选择导入模式', async ({ page }) => {
      ...
      if (await modeSelector.first().isVisible()) {
        ...
        // 至少有一种模式可见
        const hasIncrementalOrFull =
          (await incrementalOption.isVisible()) || (await fullOption.isVisible())
    ...
    test('上传区域正确显示', async ({ page }) => {
      ...
      // 验证上传区域存在（el-upload 拖拽区；其内部 input[type=file] 为隐藏元素，不可作为可见性断言目标）
      const uploadArea = page.locator('.el-upload .el-upload-dragger')
    ...
    test('可以刷新导入历史', async ({ page }) => {
      ...
      const refreshButton = page.locator(
        'button:has-text("刷新"), button:has-text("Refresh"), [aria-label*="refresh"]'
      )
  ...
  test.describe('数据导出流程', () => {
    test('可以打开导出对话框', async ({ page }) => {
      ...
      const exportButton = page.locator(
        'button:has-text("导出"), button:has-text("Export"), [aria-label*="export"]'
      )
    ...
    test('可以执行数据导出', async ({ page }) => {
      ...
      if (await exportButton.first().isVisible()) {
        ...
        const confirmButton = page
          .locator('button:has-text("开始导出"), button:has-text("确定"), button:has-text("导出")')
          .last()
    ...
    test('异步导出显示任务状态', async ({ page }) => {
      ...
      if (await exportButton.first().isVisible()) {
        ...
        if (await asyncSwitch.isVisible()) {
          ...
          // 执行导出
          const confirmButton = page
            .locator('button:has-text("开始导出"), button:has-text("确定")')
            .last()
          if (await confirmButton.isVisible()) {
            await confirmButton.click()

            // 等待任务状态显示（可能显示任务ID或状态，此处仅等待不强行断言）
            await page.waitForTimeout(1000)
          }
        }
      }
