...
test.describe('核心业务流', () => {
  ...
  test('1. 登录 → 工作台加载', async ({ page }) => {
    await expect(page.locator('.kpi-cards, .dashboard, .home-safe, #app')).toBeVisible({
      timeout: 8000,
    })
  })
  ...
  test('4. 项目列表 → 新增项目', async ({ page }) => {
    ...
    const createBtn = page
      .locator('button:has-text("新增"), button:has-text("创建"), button:has-text("添加")')
      .first()
  ...
  test('6. 数据导出对话框', async ({ page }) => {
    ...
    await page.waitForLoadState('networkidle')
    await expect(page.locator('form, .el-form, .export-form, .empty')).toBeVisible({
      timeout: 5000,
    })
  })
