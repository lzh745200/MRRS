...
describe('日志回退分支补充', () => {
  ...
  it('fetchSystemLogs {items:[...]} 信封 → 解析', async () => {
    ...
    mockGet.mockImplementation((url: string) => {
      if (url === '/system/admin/logs')
        return Promise.resolve({
          items: [{ line: '2026-08-11 20:31:46 [INFO] app.request: GET /health' }],
        })
      if (url === '/system/snapshot') return Promise.resolve({ data: mockSnapshotData })
      if (url === '/system/api-stats') return Promise.resolve({ data: mockApiStatsData })
      if (url === '/system/health') return Promise.resolve({ data: mockHealthData })
  ...
  it('真实日志格式 [INFO]/[WARNING] → 正确解析级别（error/warn 双 false 分支）', async () => {
    ...
    expect(logs.some((l: any) => l.level === 'info' && l.message.includes('GET /health'))).toBe(
      true
    )
...
describe('日志形态收尾', () => {
  ...
  it('fetchSystemLogs 数组 payload / ERROR 日志 / 未匹配行', async () => {
    ...
    mockGet.mockImplementation((url: string) => {
      if (url === '/system/admin/logs')
        return Promise.resolve([{ line: '2026-01-01 10:00:00 ERROR - 崩溃' }, '无法解析的一行'])
...
describe('API统计字段形态2', () => {
  ...
  it('avg_time_ms 有值 → 图表使用', async () => {
    mockApiRequest.mockReset()
    mockApiRequest.mockImplementation((config: any) => {
      if (config?.url === '/system/monitor/api-stats')
        return Promise.resolve({
          data: {
            data: { top_endpoints: [{ method: 'GET', path: '/a', count: 1, avg_time_ms: 2.5 }] },
          },
        })
      return Promise.resolve({})
    })
